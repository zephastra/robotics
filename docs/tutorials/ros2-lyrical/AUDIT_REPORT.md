...
1. 依据 ROS 2 官方入门教程整理的中文教程（01–06）。
...
`tutorials` 作为正式内容类别；每篇通过 Verified、Source Reviewed 或 Draft 表明验证程度。第 11 篇不构成教程，单独放入 `project-notes`。这既保持正式的仓库命名，也避免把未验证章节或项目记录误称为已完成教程。
